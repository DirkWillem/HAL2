...
export module vrpc.uart.client:transport;
...
export template <std::size_t RSC>
...
export enum class RequestError {
  ServiceIdMismatch,
  CommandIdMismatch,
  RequestIdMismatch,
...
class ClientTransport {
  ...
  struct RequestSlot {
    ...
    hstd::Callback<ResponseResult>*      callback{nullptr};
 ...
 public:
  ...
  bool Request(std::size_t slot_id, uint32_t service_id, uint32_t command_id,
               const Req& request, hstd::Callback<ResponseResult>& callback) {
  ...
  bool Request(uint32_t slot_id, uint32_t server_address, uint32_t service_id,
               ...
               hstd::Callback<ResponseResult>& callback) {
  ...
  void HandleResponses() noexcept {
    if (busy.test_and_set()) {
      if (has_response.test()) {
        hstd::ClearFlagAtExit clear_busy{busy};
        hstd::ClearFlagAtExit clear_has_response{has_response};
 ...
 private:
  ...
  ActivateRequestSlot(std::size_t slot_id, uint32_t service_id,
                      uint32_t command_id, const Req& request,
                      hstd::Callback<ResponseResult>& callback) noexcept {
    ...
    return hstd::ExclusiveWithAtomicFlag(slot.active, [&] -> bool {
      ...
      if (enc_success) {
        ...
        hstd::PendEvent(slot.tx_pending);
      }
  ...
  ActivateRequestSlot(std::size_t slot_id, uint32_t server_address,
    ...
    return hstd::ExclusiveWithAtomicFlag(
               slot.active,
               [&] -> bool {
                 slot.callback    = &callback;
  ...
  void TrySendNextRequest() noexcept {
    const auto any_sent =
        hstd::ExclusiveWithAtomicFlag(busy, [&, this] -> bool {
          for (auto& slot : request_slots) {
            if (hstd::TestAndHandleEvent(slot.tx_pending)) {
              active_slot = &slot;
  ...
  hstd::MethodCallback<ClientTransport, std::span<std::byte>> raw_rx_callback;
  hstd::MethodCallback<ClientTransport>                       tx_callback;
