...
export module vrpc.server;

import hstd;

export import vrpc.common;

namespace vrpc {

export template <typename Response>
class AsyncResult {
  struct State {
    std::reference_wrapper<Response>         response;
    std::reference_wrapper<hstd::Callback<>> callback;
  };
 ...
 public:
   ...
   */
  AsyncResult(Response& response, hstd::Callback<>& callback) noexcept
      : state{
